...
export const CHANGE_DESCRIPTION_BIO_WIZARD = 'CHANGE_DESCRIPTION_BIO_WIZARD';

@Wizard(CHANGE_DESCRIPTION_BIO_WIZARD)
@UseFilters(InvalidAnswerFilter)
export class ChangeDescriptionBioWizard {
  constructor(
  ...
  async step1(@Ctx() ctx: MainWizardContext) {
    ...
    await ctx.reply(`${questionnaire.bio}`);
    ctx.wizard.next();
  }
